...
export default function CropTool() {
  ...
  const startCrop = () => {
    ...
    if (!img) {
      alert("No image found on canvas");
      return;
    }

    // Create crop rectangle
    const rect = new fabric.Rect({
      left: img.left + img.width * img.scaleX * 0.1,
      top: img.top + img.height * img.scaleY * 0.1,
  ...
  const applyCrop = () => {
    ...
    const left = (rect.left - img.left) / img.scaleX / zoom;
    const top = (rect.top - img.top) / img.scaleY / zoom;
    const width = rect.width / img.scaleX / zoom;
    const height = rect.height / img.scaleY / zoom;
  ...
  return (
    <div className="p-4 space-y-4 text-white">
      ...
      <p className="text-sm opacity-70">Crop and trim your image</p>
